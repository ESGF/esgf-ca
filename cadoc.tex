...
The CA/CPS is designed so that a Relying Party can look at them and obtain an understanding of the trustworthiness of credentials issued by the ESGF CAs.
...
It is the responsibility of each of the ESGF CA Root CAs to publish the
...
\item
  ...
  All certificates issued by the ESGF CA Root CA and their status;
...
The ESGF CA Root CA grants the ESGF the right of unlimited
...
Furthermore, the ESGF CA Root CA requires, as a condition for
certificate issuance, that:

\begin{enumerate}
\item
  
  All Subject CAs make available to the ESGF CA Root CA results of CA
  audits and plans to remedy deficiencies
  
\item
  
  All Subject CA Managers and Operators agree to be signed up to a
  closed mailing list, maintained by the ESGF CA Root CA;
...
\item
  ...
  The ESGF CA Root CA has otherwise determined the need for revocation,
  e.g., if the Subject CA does not comply with the requirements imposed
  on it by the ESGF CA Root.
  
\end{enumerate}

\section{Certificate life-cycle and operational requirements}\label{certificate-life-cycle-operational-requirements}
...
namespace of the Subject CA with the Manager of the ESGF CA Root CA,
...
When the ESGF CA Root CA Manager is satisfied that Subject CA will
operate within the constraints imposed by the Root, The ESGF CA Root CA
will issue and publish the certificate of the Subject CA.

\subsection{Certificate issuance}\label{certificate-issuance}

The ESGF CA Root CA Manager makes the Subject CA available on its
website.

\subsection{Certificate acceptance}\label{certificate-acceptance}

Both the ESGF CA Root CA Manager and the ESGF CA Subject CA Manager
...
shall be revoked by the ESGF CA Root CA, and reissued with changes,
...
facility and ESGF CA Root CA operation.
...
The Root CA's key pair shall be generated with sufficient entropy, by the CA. It shall be the responsibility of a ESGF CA Manager to generate the key pair. The Root key pair shall be RSA and have a length of at least 2048 bits. Key pairs for Subject CAs are
...
\item No other ports on the server should be open from any machine, including the gateway host machine. This ensures that the ESGF CA Root CA server is disconnected from the internet and all other machines, except the host machine, at all times, other than the time when it is being updated. 
...
revocation lists issued by the ESGF CA Root CA. The ESGF CA Root CA
does not currently provide OCSP support.

\subsection{Certificate profile}\label{certificate-profile}

All certificates issued by the ESGF CA Root CA conform to the Internet
...
\item
  ...
  The signature algorithm shall be sha256WithRSAEncryption, or sha1WithRSAEncryption;
...
The ESGF CA Root CA will follow best practices to protect any
...
The ESGF CA Root CA does not process any personal data, except for the
following:
...
Statement'' by the UK e-Science CA run by CCLRC. The ESGF CA Root CA does not claim any IPR on certificates that it has issued. Anybody may
freely copy from any version of the ESGF CA Root CA's Certificate
...
When issuing a certificate to a Subject CA, the ESGF CA Root CA will
...
undertaking in relation to the ESGF CA Root CA and its operation.
...
The ESGF CA Root CA also declines any liability for damages arising
...
The ESGF CA Root CA declines any payment of indemnities for damages
...
All agreements between the ESGF CA Root CA and any organization must be
...
The ESGF CA Root CA shall communicate amendments to Subject CAs and to
the ESGF.
